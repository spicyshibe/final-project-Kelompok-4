...
function createReservation(req, res) {
  const { user_id, tanggal, jam, jumlah_orang } = req.body;
  if (!user_id || !tanggal || !jam || !jumlah_orang) {
    return sendResponse(res, { code: 400, success: false, message: 'user_id, tanggal, jam, jumlah_orang wajib diisi' });
  }
  ...
  const reservation = reservationModel.create({ user_id, tanggal, jam, jumlah_orang });
